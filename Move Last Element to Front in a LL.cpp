...
LinkedListNode<int>* moveToFront(LinkedListNode<int> *head)
{
    ...
    // Take a pointer 'secLast', the will point second last node of linked list 
    LinkedListNode<int> *secLast = NULL;
    ...
    {   
        ...
        // Update 'last', move to next node
        last = last->next;
    }

    // Set 'secLast' node as last node by making secLast->next as NULL
    secLast->next = NULL;

    // Link initial head, next to last node
    last->next = head;
